  ...
  public RunConfigurationContext getTestContext(ConfigurationContext context) {
    JSFile file =
        Optional.of(context)
            ...
            .filter(f -> f.isTestFile() || hasTopLevelTests(f) || isClosureTestSuite(f))
            .orElse(null);
    if (file == null) {
      return null;
    }
    ListenableFuture<TargetInfo> target = TestTargetHeuristic.targetFutureForPsiElement(file, null);
    if (target == null) {
      return null;
    }
